...
air_conditioner_control = ctrl.ControlSystem(AIR_CONDITIONER_RULES)
...
def compute(temperature: float, humidity: float, target_temperature: float) -> tuple[float, float]:
    air_conditioner.input["temperature"] = temperature
    air_conditioner.input["humidity"] = humidity
    air_conditioner.input["target_temperature"] = target_temperature
    air_conditioner.compute()

    fan_speed: float = air_conditioner.output["fan_speed"]
    mode: float = air_conditioner.output["mode"]
    ...
    for temp in temperatures:
        for hum in humidities:
            for target in targets:
                mode, fan_speed = compute(temp, hum, target)
                mode_str = format_mode(mode)
                print(f"{temp:6.1f} {hum:6.1f} {target:8.1f} {mode_str:>12} {fan_speed:12.2f}")
