...
def test_find_tokens_to_capture_at_the_bottom() -> None:
    """Should return None when there are no tokens below the given index"""

    game = BestGameEverMade([])
    game.board[0] = [BoardElement.WHITE]
    assert game._find_tokens_to_capture(game.board[0], 0) is None


def test_find_tokens_to_capture_too_low() -> None:
    """Should return None when there are too little tokens below the given index"""

    game = BestGameEverMade([])
    game.board[0] = [BoardElement.WHITE, BoardElement.BLACK]
    game.current_player = 2
    assert game._find_tokens_to_capture(game.board[0], 1) is None


def test_find_tokens_to_capture_no_consecutive_tokens() -> None:
    """Should return None when there are no consecutive opponent tokens below the given index"""

    game = BestGameEverMade([])
    game.board[0] = [BoardElement.WHITE, BoardElement.BLACK, BoardElement.WHITE]
    assert game._find_tokens_to_capture(game.board[0], 2) is None


def test_find_tokens_to_capture_consecutive_tokens() -> None:
    """Should return captureable indexes when there are 2 consecutive opponent tokens below the given index"""

    game = BestGameEverMade([])
    game.board[0] = [BoardElement.WHITE, BoardElement.WHITE, BoardElement.BLACK]
    game.current_player = 2
    assert game._find_tokens_to_capture(game.board[0], 2) == [0, 1]
...
def test_find_tokens_to_score_three_tokens_to_score() -> None:
    ...
    game = BestGameEverMade([])
    game.board[0] = [BoardElement.WHITE, BoardElement.WHITE, BoardElement.WHITE]
    assert game._find_tokens_to_score(game.board[0]) == 3
...
def test_find_tokens_to_score_four_tokens_to_score() -> None:
    ...
    assert game._find_tokens_to_score(game.board[0]) == 4
